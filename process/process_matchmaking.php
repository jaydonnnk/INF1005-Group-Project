...
switch ($action) {
    ...
    case "update":
        ...
        $errors = [];
        if (empty($title) || strlen($title) > 80)       $errors[] = "Title is required (max 80 chars).";
        ...
        elseif (strtotime("{$session_date} {$session_time}:00") <= time())
                                                         $errors[] = "Session date and time cannot be in the past.";
        if ($spots_total < 1 || $spots_total > 10)       $errors[] = "Spots must be 1–10.";
    ...
    case "join":
        ...
        $post_id = (int) ($_POST["post_id"] ?? 0);
        if ($post_id <= 0) { setFlash('error','Invalid session.'); header("Location: ../matchmaking.php"); exit(); }
        ...
        if (!$row || $row['status'] !== 'Open') { setFlash('error','Session not found or closed.'); header("Location: ../matchmaking.php"); exit(); }
        if ((int)$row['member_id'] === $member_id) { setFlash('error','You cannot join your own session.'); header("Location: ../matchmaking.php"); exit(); }
        if ((int)$row['spots_filled'] >= (int)$row['spots_total']) { setFlash('error','This session is already full.'); header("Location: ../matchmaking.php"); exit(); }
        ...
        } catch (Exception $e) { error_log("Join error: ".$e->getMessage()); setFlash('error','Could not join. Please try again.'); }
        header("Location: ../matchmaking.php"); exit();
    ...
    case "unjoin":
        ...
        } catch (Exception $e) { error_log("Unjoin error: ".$e->getMessage()); setFlash('error','Could not leave. Please try again.'); }
        header("Location: ../matchmaking.php"); exit();
    ...
    case "uninterest":
        ...
        } catch (Exception $e) { error_log("Uninterest error: ".$e->getMessage()); setFlash('error','Could not remove interest. Please try again.'); }
        header("Location: ../matchmaking.php"); exit();
